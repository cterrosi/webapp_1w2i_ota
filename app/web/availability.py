...
def departures_by_dest():
    ...
    base_sql = """
        SELECT depart_date, MAX(COALESCE(duration_days,0)) AS duration_days
    ...
    def iso10(v):
        try:
            return v.date().isoformat()   # datetime/date -> 'YYYY-MM-DD'
        except Exception:
            return str(v)[:10]

    out = [{"date": iso10(d), "duration_days": int(dur or 0)} for d, dur in rows]
